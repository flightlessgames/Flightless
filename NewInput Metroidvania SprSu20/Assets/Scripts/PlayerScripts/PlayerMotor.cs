...
public class PlayerMotor : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        input.PlayerControls.Movement.performed += ctx => rawMovementInput = ctx.ReadValue<Vector2>();
    ...
    private void Climb()
    {
        ...
        bool canClimb = false;
        //class interfacer is an empty base class, childed is isFloor, isWall
        ClassInterfacer[] classesInterfaced = CheckWalls();
        //checkWalls checks for collisions with Walls, Floors
        //create direction storage for UP
        WallCheckDirection direction = WallCheckDirection.Up;
        //check movement, if <0 moving LEFT and check array[0] for first check (hard-coded as Left)
        if (movementInput.x < 0)
        {
            if (classesInterfaced[0] is IClimbable)
            {
                ...
                //set direction storage for Left
            }
        }
        //if movement >0 moving RIGHT and array[1] is hard-coded right
        if (movementInput.x > 0)
        {
        ...
        //canClimp only set to true if moving BOTH a direction and UP
        if (canClimb)
        {
    ...
    private IEnumerator PlayClimbAnimation(WallCheckDirection direction)
    {
        ...
        //settings for automated override, to play climbing animation (not ladder climbable, only "climb" like Free-Running/Parkour)
        isControl = false;
        rb.useGravity = false;
